...
int main(int argc, char *argv[]) {
    // Verifica se o usurário passou 4 argumentos para o correto funcionamento do código
    // Argumentos a serem passados: ./@file <hash> <pass_len> <charset> <num_workers>
    if (argc != 5) {
        fprintf(stderr, "Uso interno: %s <hash> <pass_len> <charset> <num_workers>\n", argv[0]);
        return 1;
    }
